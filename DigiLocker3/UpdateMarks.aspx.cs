    ...
    public partial class UpdateMarks : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                table_name = ddlCourseType.SelectedValue.Replace(" ", "_") + "_" + ddlCourseNo.SelectedValue + "_" + ddlEntryType.SelectedValue.Replace(" ", "_") + "_" + "SUBJECTS";
            ...
            if (ddlSubject.SelectedValue.Equals("Seniority"))
            {
                ...
                else
                {
                    ...
                    else if (markspresent < (55.0 * max_marks) / 100.0)
                    {
                        ...
                        if (marks >= (55.0 * max_marks) / 100.0)
                        {
                            query = "update " + table_name + " set " + term + "_Failed = ( WHEN " + term + "_failed > 0 THEN " + term + "_failed - 1 END) where Personal_No = '" + Personal_No + "'";
                            cmd = new SqlCommand(query, con);
                            cmd.ExecuteNonQuery();
                        }
        ...
        protected void ddlCourseTypeIndexChanged(object sender, EventArgs e)
        {
            ...
            string term = ddlTerm.SelectedValue;
            string table_name = ddlCourseType.SelectedValue.Replace(" ", "_") + "_" + ddlCourseNo.SelectedValue + "_" + ddlEntryType.SelectedValue.Replace(" ", "_") + "_" + "SUBJECTS";
