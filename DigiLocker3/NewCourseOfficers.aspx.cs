...
{
    public partial class NewCourseOfficers : System.Web.UI.Page
    {
        string name;
        static int flag1;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                flag1 = 0;
                con.Open();
                SqlCommand com = new SqlCommand("select CONCAT(COURSE_NAME, ' ', COURSE_NO) AS TYPE_NAME from OFFICER_COURSE", con); // table name 
                ...
                com = new SqlCommand("select * from OFFICER_COURSE_TYPE", con); // table name 
            ...
            if (flag1 == 0)
            {
                ...
                else if (!System.Text.RegularExpressions.Regex.IsMatch(Course_Number_TextBox.Text, "[^a-zA-Z.0-9\x20]", System.Text.RegularExpressions.RegexOptions.IgnoreCase) & Course_Number_TextBox.Text[0] != ' ' & Course_Number_TextBox.Text[0] != '\t')
                {
                    ...
                    try
                    {
                        ...
                        string insertQuery;
                        SqlCommand cmd;
                        insertQuery = "insert into Officer_Course(Course_No, Course_Name, StartDate, EndDate)values (@Course_No, @Course_Name, @StartDate, @EndDate)";
