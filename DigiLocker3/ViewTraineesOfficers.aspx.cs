...
{
    public partial class ViewTraineesOfficers : System.Web.UI.Page
    {
        string coursename;
        ...
        protected void ddlCourseTypeIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlCourseType.SelectedValue.Equals("0"))
            {
                ...
                SqlCommand com = new SqlCommand("select Course_No from officer_Course where Course_Name = '" + coursename + "'", con); // table name 
                SqlDataAdapter da = new SqlDataAdapter(com);
                DataSet ds = new DataSet();
                da.Fill(ds);  // fill dataset
                ddlCourseNo.DataTextField = ds.Tables[0].Columns["Course_No"].ToString(); // text field name of table dispalyed in dropdown
            ...
            try
            {
                ...
                if (sqlRdr.HasRows)
                {
                    while (sqlRdr.Read())
                    {
                        ...
                        Response.Redirect("ViewTraineesofficers.aspx", false);
